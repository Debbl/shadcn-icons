...
for (const collection of Object.keys(Collections)) {
  ...
  const json = {
    ...
    icons: Object.fromEntries(
      Object.entries(iconsJson.icons).map(([key, value]) => {
        const alias = Object.entries(iconsJson.aliases ?? {}).find(([_, v]) => v.parent === key)?.[0] ?? key
  ...
  writeFileSync(path.join(ICON_SETS_DIR, `${collection}.json`), JSON.stringify(iconSet, null, 0))
}
